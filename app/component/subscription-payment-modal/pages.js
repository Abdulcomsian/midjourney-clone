...
function SubcriptionModal() {
    ...
    const {paymentMethod,errorPaymentMethod,paymentInitializationURL } = useSelector((state) => state.api);
    ...
    const paymentIntilaziationHandling = () => {
        dispatch(paymentInitialization({
                ...
                {errorPaymentMethod ? (
                    ...
                    paymentMethod.paymentMethod.methods.map((item,index) => (
                        ...
                        <Form.Check // prettier-ignore
                            ...
                            label={item.name}
                            className='mb-3'
                            onChange={() => handlePaymentMethodChange(item.name.toLowerCase())}
                        />
                        </div>
                    ))
                ):(
                    <p>No Payment is availabel for you country</p>
                )}
                <Button className='w-100 btn-warning' onClick={paymentIntilaziationHandling}>Pay Now</Button>
            </Modal.Body>
        </Modal>
    )
}
